 ...
 */
const createTechnicalSupportIntoDB = async (
  user: JwtPayload,
  payload: ITechnicalSupportRoutes
) => {
 ...
 */
const updateTechnicalSupportFromDB = async (
  id: string,
  payload: ITechnicalSupportRoutes
) => {
  const result = await TechnicalSupportModal.findByIdAndUpdate(id, payload, {
    new: true,
  });
  if (!result) {
    throw new ApiError(
